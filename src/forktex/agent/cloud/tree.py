...
async def tree(ctx, org):
    ...
    verified_str = click.style("✓", fg="green") if email_verified else click.style("✗", fg="red")

    click.echo()
    click.echo(f"account: {click.style(email, bold=True)}  (email_verified: {verified_str})")
    click.echo()
    ...
    for o in orgs:
        ...
        click.echo(f"└── org: {click.style(o_slug, fg='cyan', bold=True)}  [{_short(o_id)}]")
        ...
        try:
            ...
            if providers:
                pstr = "  ".join(
                    f"{click.style(p.provider, fg='green')} ✓" if p.is_active else p.provider
                    for p in providers
                )
        ...
        # Projects
        org_projects = [p for p in projects if str(getattr(p, "orgId", getattr(p, "org_id", None))) == str(o_id)]
        if not org_projects:
            click.echo("    │")
        ...
        for pi, project in enumerate(org_projects):
            ...
            click.echo(f"{p_prefix} project: {click.style(p_name, bold=True)}  [{_short(p_id)}]")
            ...
            for ei, env in enumerate(envs):
                ...
                env_server = next(
                    (s for s in servers if str(getattr(s, "environmentId", getattr(s, "environment_id", None))) == str(e_id)),
                    None,
                )
                server_str = ""
                if env_server:
                    s_id = getattr(env_server, "id", "?")
                    s_ip = getattr(env_server, "ip", None) or getattr(env_server, "ipv4", "?")
                    s_name = getattr(env_server, "name", str(s_id)[:8])
                    server_str = f"  server: {click.style(s_name, fg='yellow')}  ip={s_ip}"
                ...
                try:
                    ...
                    if deployments:
                        ...
                        click.echo(f"{e_cont}├── last deploy: {click.style(d_status, fg=d_color)}  {d_str}")
                except Exception:
                    pass
                ...
                if env_server:
                    try:
                        ...
                        for si, svc in enumerate(services):
                            ...
                            svc_prefix = f"{e_cont}└──" if is_last_svc else f"{e_cont}├──"
