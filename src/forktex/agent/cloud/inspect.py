...
async def inspect_org(ctx):
    ...
    if providers:
        ...
        for p in providers:
            status = click.style("✓ active", fg="green") if p.is_active else click.style("revoked", fg="red")
            env_tag = f"  [{p.environment}]" if p.environment else ""
            click.echo(f"    {p.provider}/{p.kind}{env_tag}  {status}  label={p.label!r}")
    else:
...
async def inspect_project(ctx, project_id):
    ...
    if not pid:
        raise click.ClickException("No project ID given and no active project set. Run: forktex cloud use project <name>")
    ...
    p_created = str(getattr(project, "createdAt", None) or getattr(project, "created_at", ""))[:16]
    ...
    for env in envs:
        ...
        click.echo(f"  ├── {click.style(e_name, bold=True)}  [{str(e_id)[:8]}]"
        ...
        try:
            ...
            if deployments:
                last = deployments[-1]
                d_status = last.get("status", "?") if isinstance(last, dict) else getattr(last, "status", "?")
                d_at = last.get("created_at", "") if isinstance(last, dict) else getattr(last, "createdAt", "")
                d_str = str(d_at)[:16].replace("T", " ") if d_at else ""
                d_color = "green" if d_status == "success" else ("red" if d_status == "failed" else "yellow")
                click.echo(f"  │   last deploy: {click.style(d_status, fg=d_color)}  {d_str}"
...
async def inspect_env(ctx, environment_id):
    ...
    if not pid:
        raise click.ClickException("No active project. Run: forktex cloud use project <name>")
    if not eid:
        raise click.ClickException("No environment ID given and no active env set. Run: forktex cloud use env <name>")
    ...
    if services:
        ...
        for svc in services:
            ...
            click.echo(f"    {click.style(svc_name, bold=True):30s}  {click.style(svc_state, fg=svc_color):10s}"
    ...
    if deployments:
        ...
        for d in reversed(deployments[-5:]):
            ...
            d_at = d.get("created_at", "") if isinstance(d, dict) else getattr(d, "createdAt", "")
            d_str = str(d_at)[:16].replace("T", " ") if d_at else ""
            d_color = "green" if d_status == "success" else ("red" if d_status == "failed" else "yellow")
            click.echo(f"    [{str(d_id)[:8]}]  {click.style(d_status, fg=d_color):12s}  {d_str}")
    click.echo()
