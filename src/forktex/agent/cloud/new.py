...
@click.option("--env", default="production", help="Environment name (default: production)")
@click.option("--list", "list_only", is_flag=True, help="List available templates and exit")
@click.pass_context
    ...
    with ForktexCloudClient.from_context(cloud_ctx) as client:
        ...
        if tmpl is None:
            ...
            try:
                ...
                raise click.ClickException(f"Template not found: {template_slug!r}. Run with --list to see available templates.")
        ...
        # Show deploy steps
        steps = getattr(tmpl, "deploySteps", None) or getattr(tmpl, "deploy_steps", []) or []
        if steps:
        ...
        required = getattr(tmpl, "requiredSecrets", None) or getattr(tmpl, "required_secrets", []) or []
        if required:
            ...
            if await click.confirm("  Set these secrets now?", default=True):
                for secret in required:
                    ...
                    default_val = _secrets.token_hex(24) if "password" in secret.lower() else ""
                    val = await click.prompt(f"  {secret}", default=default_val, hide_input="password" in secret.lower())
                    if val:
        ...
        if domain:
            overrides = {
                "cloud": {
                    "gateway": {
                        "domains": [{"host": domain, "primary": True}]
                    }
                }
            }
        ...
        environment_id = getattr(resp, "environmentId", None) or getattr(resp, "environment_id", "")
...
def _print_template_list(templates) -> None:
    ...
    for t in templates:
        slug = t.slug
        name = t.name
        flavour = getattr(t, "recommendedFlavour", None) or getattr(t, "recommended_flavour", "?") or "?"
        cost = getattr(t, "estimatedMonthlyCostUsd", None) or getattr(t, "estimated_monthly_cost_usd", None)
        bundle = "📦 " if (getattr(t, "hasCodeBundle", False) or getattr(t, "has_code_bundle", False)) else "   "
        cost_str = f"~${cost:.2f}/mo" if cost else ""
        click.echo(f"  {bundle}{click.style(slug, fg='cyan', bold=True):30}  {name:30}  {flavour:12}  {cost_str}")
    click.echo()
...
def _print_template_detail(tmpl) -> None:
    ...
    flavour = getattr(tmpl, "recommendedFlavour", None) or getattr(tmpl, "recommended_flavour", "")
    cost = getattr(tmpl, "estimatedMonthlyCostUsd", None) or getattr(tmpl, "estimated_monthly_cost_usd", None)
