...
# Knowledge + working-memory tools (the active side of the knowledge loop —
# distinct from the passive grounding injected into the system prompt). Read +
# remember are safe for any investigating agent; recycling a durable lesson is
# reserved for agents that also change the codebase.
_KNOWLEDGE_TOOLS = frozenset({"knowledge_search", "knowledge_show"})
_MEMORY_TOOLS = frozenset({"remember"})
_RECYCLE_TOOLS = frozenset({"knowledge_recycle"})
...
#
# forktex picks its own agent from the task — there is no manual `-t researcher`
# interface on the primary path. Heuristic today (model-independent, so it works
# even against a weak backend); an LLM-routed upgrade rides the one-shot
# completion seam once a capable model is wired.

_BUILD_RE = re.compile(
    r"\b(add|fix|implement|refactor|rewrite|write|create|update|rename|delete|"
    r"remove|migrate|patch|edit|wire|build|scaffold|generate|set up|install)\b",
    re.IGNORECASE,
)
_CODE_REVIEW_RE = re.compile(
    r"\bcode[- ]?review\b|"
    r"\b(review|audit)\b[\w\s]{0,20}?\b(diff|pr|pull request|commit|code|changes|security)\b",
    re.IGNORECASE,
)
_RESEARCH_RE = re.compile(
    r"\b(what|which|how|why|where|who|find|search|analy[sz]e|investigate|explain|"
    r"explore|understand|summar|assess|report|list|inspect|look (at|into)|"
    r"review (the )?(state|status|docs|documentation|repo))\b",
    re.IGNORECASE,
)


def route_agent_type(task: str) -> str:
    """Pick an agent type from the task — no manual ``--agent-type`` needed.

    Precedence: a build/change verb wins (it implies write access), then a
    code/diff review, then research/read intent; otherwise the general
    all-tools ``assistant`` (the "one agent").
    """
    text = task or ""
